...
function Reviews() {
    ...
    useEffect(() => {
        ...
        if (house == -1) {
            async function getData() {
                let res = await axios.get(API_HOST + `user/client/detail/review/by-guest?page=0&size=${size}`,
                    {
                        headers: {
                            'Authorization': `Bearer ${localStorage.getItem("jwt")}`
                        }
            ...
            async function getData() {
                let res = await axios.get(API_HOST + `house/detail/reviews/${house}?page=0&size=${size}`,
                    {
                        headers: {
                            'Authorization': `Bearer ${localStorage.getItem("jwt")}`
                        }
                                                ...
                                                <>
                                                    <div className='border-bottom col-8'>
                                                        ...
                                                        <div style={{ paddingBottom: '50px' }}>
                                                            <div style={{ marginLeft: '600px', marginBottom: '100px' }} className='text-decoration-underline  mb-3' onClick={() => {
                                                                !choose.includes(item?.review?.id) ? setChoose([...choose, item?.review?.id]) : setChoose(choose.filter(function (number) {
                                                    ...
                                                    listReview.map((item) => (
                                                        <>
                                                            <div className='border-bottom col-8'>
                                                                ...
                                                                <div style={{ paddingBottom: '50px' }}>
                                                                    <div style={{ marginLeft: '600px', marginBottom: '100px' }} className='text-decoration-underline  mb-3' onClick={() => {
                                                                        !choose.includes(item?.id) ? setChoose([...choose, item?.id]) : setChoose(choose.filter(function (number) {
