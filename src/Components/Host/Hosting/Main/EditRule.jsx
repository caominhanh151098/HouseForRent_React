...
function EditRule() {
    ...
    useEffect(() => {
        async function getData() {
            let res = await axios.get(API_HOST + `rule/getRuleBoolen`,
                {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem("jwt")}`
                    }
    ...
    useEffect(() => {
        async function getData() {
            let res = await axios.get(API_HOST + `rule/getRule/${houseID}`,
                {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem("jwt")}`
                    }
    ...
    const handleUpdateBookNow = () => {
        async function getData() {
            let res = await axios.get(API_HOST + `house/setBookNow/${houseID}`,
                {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem("jwt")}`
                    }
    ...
    const handleUpdateRuleBoolen = (ruleID, status) => {
        async function getData() {
            let res = await axios.get(API_HOST + `house/updateRuleBoolen/${houseID}/${ruleID}/${status}`,
                {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem("jwt")}`
                    }
    ...
    const handleUpdateQuietTime = (ruleID, startTime, endTime) => {
        async function getData() {
            let res = await axios.get(API_HOST + `rule/updateQuietTime/${houseID}/${startTime}/${endTime}/${ruleID}`,
                {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem("jwt")}`
                    }
    ...
    const handleUpdateOther = () => {
        ...
        async function getData() {
            ...
            let res2 = await axios.post(API_HOST + `rule/updateOther/${houseID}`, { stringRequest: other }, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem("jwt")}`
                }
            });
