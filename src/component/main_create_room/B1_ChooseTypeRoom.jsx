 ...
 function B1_ChooseTypeRoom  ()  {
  ...
  return (
   ...
   <div className='fixed-bottom d-flex justify-content-between'>
    <Link className="btn btn-sm ms-5 ms-5 mb-5 fs-5" to={'/create/b1/chooseType'}>
                            <i className="fa fa-arrow-left me-2" />
                            quay lại
                        </Link>
                        <div><Link className="" to={'/create/b1/address '}> <button onClick={()=>{CreateRoom.setCreateRoom({...CreateRoom.getCreateRoom(), typeRoom:roomType})}} className={`btn bg-dark text-white me-5 mb-5 ${roomType==''?'disabled':''}`} >Tiếp theo</button></Link></div>
    </div>
   </>
  )
