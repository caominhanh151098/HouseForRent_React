...
function B3_discount (){
  ...
  return (
    ...
    <Link className="btn btn-sm ms-5 ms-5 mb-5 fs-5" to={'/create/b3/price'}>
                            <i className="fa fa-arrow-left me-2" />
                            quay lại
                        </Link>
                        <div><Link className="" to={'/create/b3/finish'}> <button onClick={()=>{CreateRoom.setCreateRoom({...CreateRoom.getCreateRoom(), discount:discount})}} className='btn bg-dark text-white me-5 mb-5' >Tiếp theo</button></Link></div>
    </div>
    </>
  )
